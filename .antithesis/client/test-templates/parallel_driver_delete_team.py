...
def do_deletes(address):
    try:
        ...
        if id is not None:
            id = id[0]
            helper.execute_psql(conn, f"DELETE FROM users WHERE team_id = {id}")
            helper.execute_psql(conn, f"DELETE FROM deployments WHERE team_id = {id}")
            helper.execute_psql(conn, f"DELETE FROM teams WHERE id = {id}")
...
def main():
    ...
    threads = []
    for i in range(helper.random_int(1, 10)):
