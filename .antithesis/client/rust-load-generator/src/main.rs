        ...
        for _ in 0..5000 {
            ...
            tokio::spawn(async move {
                ...
                for letter in letters.chars() {
                    match client.query_typed::<TeamStats>(&Statement::WithParams(TEAM_QUERY.into(), vec![format!("%{}%", letter).into()])).await {
                        Ok(mut res) => {
...
async fn subscribe(addr: String, mut tripwire: Tripwire) -> eyre::Result<()> {
    ...
    loop {
        ...
        let client: &CorrosionClient = corro_client.as_ref().unwrap();
        let mut sub: corro_client::sub::SubscriptionStream<DeploymentStats> = match if let Some(sub_id) = sub_id {
            client
...
async fn create_client(addr: &str) -> Result<CorrosionClient> {
    ...
    match system_resolver.lookup_ip(host_name).await?.iter().next() {
        Some(ip_addr) => {
            let addr = SocketAddr::from((ip_addr, host_port.next().unwrap_or("8080").parse().unwrap()));
            let corro_client = CorrosionClient::new(addr, "/var/lib/corrosion2/state.db");
            Ok(corro_client)
        }
        None => {
            Err(eyre!("could not resolve ip address for {}", addr))
        }
    }
}   
