...
use crate::pubsub::{unpack_columns, MatchCandidates, MatcherError, MatcherCreated};
use crate::schema::Schema;
use async_trait::async_trait;
use corro_api_types::{Change, QueryEvent, SqliteValueRef, TableName};
...
struct InnerUpdatesManager {
    ...
    handles: HashMap<Uuid, UpdateHandle>,
}
...
impl UpdatesManager {
    ...
    pub fn match_changes(&self, changes: &[Change], db_version: CrsqlDbVersion) {
        ...
        for (id, handle) in handles.iter() {
            ...
            for change in changes.iter().map(UpdatableChange::from) {
                if handle.filter_updatable_change(&mut candidates, change) {
                    match_count += 1;
                }
            }
...

        {
            ...
            for change_res in rows {
                let (table, pk) = change_res?;

                for (_id, (candidates, handle)) in candidates.iter_mut() {
                    let change = UpdatableChange {
                        table: &table,
                        pk: &pk,
                    };
                    handle.filter_updatable_change(candidates, change);
                }
            }
        }
        ...
        for (id, (candidates, handle)) in candidates {
            ...
            for (table, pks) in candidates.iter() {
                let count = pks.len();
                match_count += count;
                counter!("corro.subs.changes.matched.count", "table" => table.to_string()).increment(count as u64);
            }
...
#[derive(Debug)]
pub struct UpdatableChange<'a> {
    table: &'a TableName,
    pk: &'a [u8],
}

impl<'a> From<&'a Change> for UpdatableChange<'a> {
...
impl UpdateHandle {
    ...
    pub fn create(
        ...
        spawn_counted(cmd_loop(id, evt_tx, changes_rx, tripwire));
        Ok(handle)
    }

    fn filter_updatable_change(
        &self,
        candidates: &mut MatchCandidates,
        change: UpdatableChange,
    ) -> bool {
...
async fn cmd_loop(id: Uuid, evt_rx: mpsc::Sender<QueryEvent>,
    mut changes_rx: mpsc::Receiver<(MatchCandidates, CrsqlDbVersion)>,
    mut tripwire: Tripwire) {
    ...
    loop {
        ...
        if process {
            ...
            if let Err(e) = block_in_place(|| {
                handle_candidates(evt_rx.clone(), std::mem::take(&mut buf))
            }) {
                if !matches!(e, MatcherError::EventReceiverClosed) {
