...
pub async fn setup(conf: Config, tripwire: Tripwire) -> eyre::Result<(Agent, AgentOptions)> {
    ...
    let (tx_apply, rx_apply) = bounded(conf.perf.apply_channel_len, "apply", None);
    let (tx_clear_buf, rx_clear_buf) = bounded(conf.perf.clearbuf_channel_len, "clear_buf", None);
    ...
    let (tx_bcast, rx_bcast) = bounded(conf.perf.bcast_channel_len, "bcast", None);
    let (tx_empty, rx_empty) = bounded(conf.perf.empties_channel_len, "empty", None);
    let (tx_changes, rx_changes) = bounded(conf.perf.changes_channel_len, "changes", None);
    let (tx_foca, rx_foca) = bounded(conf.perf.foca_channel_len, "foca", None);
