...
mod tests {
    ...
    async fn test_lagging_subscribers() -> eyre::Result<()> {
        ...
        // Ensure the writes are flushed to the subscription
        ...
        assert_eq!(events.len(), data.len() + 2); // data + columns + end of query
        ...
        // Try for the subscription to get purged, s2 and s3 should not get disconnected
