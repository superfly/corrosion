        ...
        for _ in 0..config.operations_per_tx {
            ...
            let stmt = if op_type < insert_pct {
                ...
                Statement::WithParams(
                    format!("INSERT OR REPLACE INTO {table_name} (id, value, counter, random) VALUES (?, ?, ?, ?)").into(),
                ...
                Statement::WithParams(
                    format!("UPDATE {table_name} SET counter = counter + 1, value = ?, random = ? WHERE id = ?").into(),
                ...
                let id = rng.random_range(1..=config.initial_rows_per_table.min(1000));
                Statement::WithParams(
                    format!("DELETE FROM {table_name} WHERE id = ?").into(),
                    vec![(id as i64).into()],
                )
            };
