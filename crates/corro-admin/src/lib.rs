                            ...
                            match booked_read.get_partial(&version) {
                                ...
                                None => {
                                    match agent.pool().read().await {
                                        Ok(conn) => match conn.prepare_cached("SELECT db_version, MAX(seq) AS last_seq FROM crsql_changes WHERE site_id = :actor_id AND db_version = :version GROUP BY true") {
